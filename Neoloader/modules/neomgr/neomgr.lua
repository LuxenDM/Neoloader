...
local re_path = lib.get_path() or lib.get_path("neomgr", "2.4.0")
local re_ver = lib.plugin_read_str(re_path .. "neomgr.lua", nil, "modreg", "version")
local re_standalone = "NO"

local cp = function(msg, v)
	lib.log_error("[neomgr] " .. tostring(msg), v or 1, "neomgr", re_ver)
end
...
local create_CCD1_view = function(id, version)
	...
	if not ctable then
		cp("ccd1 doesn't provide smart_config, aborting")
		return
	end
		...
		local create_setting_editor = function(setting_to_edit)
			...
			if not rules.type then
				...
				local cfg_button = iup.stationbutton {
					...
					action = function(self)
						local new_setting = (cur_select == rules.valid[1]) and rules.valid[2] or rules.valid[1]
			...
			iup.vbox {
				...
				iup.hbox {
					...
					iup.stationbutton {
						...
						size = "x" .. button_scalar(),
						action = function()
							Game.OpenWebBrowser("https://www.moddb.com/mods/neoloader/")
						end,
					},
					iup.fill { },
				},
					...
					iup.vbox {
						...
						iup.hbox {
							...
							iup.stationbutton {
								...
								fgcolor = "255 0 0",
								action = function()
									lib.request_auth("neomgr (LME Removal Request)", lib.uninstall)
								end,
							},
						},
					},
